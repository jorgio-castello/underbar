(function() {
  ...
  _.map = function(collection, iterator) {
    ...
    let mappedArr = [];
    _.each(collection, function(item, index) {
      mappedArr.push(iterator(item, index, collection));
    });
    return mappedArr;
  };
  ...
  _.memoize = function(func) {
    ...
    return function() {
      ...
      let parameterString = JSON.stringify(arguments);
      //Conditional statement: does the string exist as a key in the object results
      if(results[parameterString] !== undefined) return results[parameterString]; //true: return the value of the property at key
      else {
          ...
          results[parameterString] = result;
          return result;
      }
    }
  };
  ...
  _.sortBy = function(collection, iterator) {
    ...
    while(!isSorted) {
      for(let i = 1; i < collection.length; i++) {
        isSorted = true;
        let isFunc = typeof iterator === 'function';
        if(isFunc && iterator(collection[i - 1]) > iterator(collection[i]) || !isFunc && collection[i - 1][iterator] > collection[i][iterator]) {
          isSorted = false;
